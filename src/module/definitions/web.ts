  ...
  actions: {
    ...
    readPage: {
      ...
      async execute({
        ...
        try {
          const maxCompletionTokens = contextWindowSize / 4;
          // console.log({ maxCompletionTokens });
          const pageSummary = await getPageSummary(maxCompletionTokens, url);
...
export async function getPageSummary(maxSummaryTokens: number, url: string) {
  const maxCompletionTokens = Math.round(contextWindowSize * 0.9);
